...
def file_reader(glue_file, base_cases_file, prior_cases_file):
    ...
    while True:
        if read_glue[0][i] != 'n':
            ...
            break
    power_of_3 = int(s)
    residue_start = (power_of_3 - 1) / 2

    beginning_length = floor(len(ast.literal_eval(odd_glue[0][0])) / power_of_3)
    base_case_start = int(read_base_cases[0])
    base_case_end = len(read_base_cases) / 4 + base_case_start

    return [odd_glue, even_glue, base_cases, power_of_3, residue_start, beginning_length, base_case_start, base_case_end, prior_cases]

def shift_sequence(seq, c, power_of_3, beginning_length):
    seq2 = []
    for i in range(len(seq)):
        seq2.append(seq[i] * power_of_3 + c * (-1)**i)
    return seq2[beginning_length - 1:]

def build_new_sequence(seq1, seq2, glue, power_of_3, beginning_length):
    new_seq = []
    for ele in glue:
        if type(ele) == int or type(ele) == sage.rings.integer.Integer:
            ...
            if 'R' in ele:
                c = int(ele[2:])
                if ele[1] == 'T':
                    shift = shift_sequence(seq1, c, power_of_3, beginning_length)
                    shift.reverse()
                else:
                    shift = shift_sequence(seq2, c, power_of_3, beginning_length)
                    shift.reverse()
            else:
                c = int(ele[1:])
                if ele[0] == 'T':
...
def previous_case(n, power_of_3, residue_start):
    a = floor(n / power_of_3)
    b = n % power_of_3
    if b < residue_start:
        return [a-1, b + power_of_3]
    else:
        return [a, b]
...
def generate_path_pair(n, read_file):
    ...
    if n < base_case_end:
        ...
        small_n = previous_case(n, power_of_3, residue_start)
        ...
        pair1 = build_new_sequence(small_seq[0], small_seq[1], glue1, power_of_3, beginning_length)
        pair2 = build_new_sequence(small_seq[0], small_seq[1], glue2, power_of_3, beginning_length)
...
def generate_path(n, read_file):
    ...
    if n < base_case_end:
        ...
        sequence = build_new_sequence(small_seq[0], small_seq[1], glue, power_of_3, beginning_length)
