...
def general_path(n, multiples):
    ...
    for i in integers:
        l = []
        for j in integers:
            if valid_sum(i, j, multiples) == True and i != j:
                l.append(j)
        d[i] = l
...
def prior_cases(stop, multiples):
    d = dict()
    for i in range(1, stop + 1):
        path = general_path(i, multiples)
        if path != False:
            d[i] = path
    return d

def path_dictionary(n, beginning, end, multiples):
    d = dict()
...
def pair_dictionary(n, beginning, end, parity_dict, multiples):
    d = dict()
    ...
    for i in integers:
        l = []
        for j in integers:
            if valid_sum(i, j, multiples) == True and i != j and parity_dict[i] != parity_dict[j]:
                l.append(j)
...
def generate_pair(n, beginning, end, multiples):
    d = path_dictionary(n, beginning, end, multiples)
    graph = Graph(d)
    while True:
        path = find_hamiltonian(graph, find_path = True)
        if path[0] != False:
            ...
            d2 = pair_dictionary(n + 1, beginning, end, parity_dict, multiples)
            graph2 = Graph(d2)
...
def create_text_file(file_name, base_case_start, base_case_end, beginning, end, multiples):
    ...
    for i in range(base_case_start, base_case_end + 1):
        base_case_pair = generate_pair(i, beginning, end, multiples)
        txt = str(i) + '\n' + str(base_case_pair[0]) + '\n' + str(base_case_pair[1]) + '\n\n'
        file.write(txt)

    file.close()
    
def create_prior_cases_text_file(file_name, stop, multiples):
    file = open(file_name, 'w')

    prior_dict = prior_cases(stop, multiples)
    for i in prior_dict:
        txt = str(i) + '\n' + str(prior_dict[i]) + '\n\n'
        file.write(txt)
...
#generate_pair: returns a nice pair for the given value of n
    #inputs
        ...
        #multiples (list): which multiples of a power of 3 should be used
    #output
