...
def even_square_sum(a, b):
    s = a + b
    if math.sqrt(s) % 2 == 0 or s == 9:
        return True
    else:
        return False
...
def general_path(n):
    ...
    for i in integers:
        l = []
        for j in integers:
            if even_square_sum(i, j) == True and i != j:
                l.append(j)
        d[i] = l
...
def path_dictionary(n, beginning, end, gap, start):
    d = dict()
    ...
    integers = list(range(start, gap * n + 1, gap))
...
def parity_dictionary(path, gap, start):
    parity = False
    d = dict()
    ...
    if len(path) % 2 == 0:
        d[gap*len(path) + start] = False
    else:
        d[gap*len(path) + start] = True

    return d

def pair_dictionary(n, beginning, end, parity_dict, gap, start):
    d = dict()
...
def generate_pair(n, beginning, end, gap, start):
    d = path_dictionary(n, beginning, end, gap, start)
    graph = Graph(d)
    while True:
        path = find_hamiltonian(graph, find_path = True)
        if path[0] != False:
            ...
            if path[0] != beginning[0]:
                path.reverse()
            parity_dict = parity_dictionary(path, gap, start)
            d2 = pair_dictionary(n + 1, beginning, end, parity_dict, gap, start)
            graph2 = Graph(d2)
...
def create_text_file(file_name, base_case_start, base_case_end, beginning, end, gap, start):
    file = open(file_name, 'w')

    for i in range(base_case_start, base_case_end + 1):
        base_case_pair = generate_pair(i, beginning, end, gap, start)
